...
% 读取标定结果文件
load('/Users/mumu/Documents/MATLAB/Polarizaiton_Imaging_Aid_Coarse_Depth/data_ball/7_2leftcalibrationSession.mat');
...
% 读取偏振去歧义后存储的.mat文件
load('/Users/mumu/Documents/MATLAB/Polarizaiton_Imaging_Aid_Coarse_Depth/data_ball/3_leftball_ODS.mat');
normal1 = double(normal1);
[Z_merged1,mask] = mergeDepthNormals(depthmap,normal1,P,k,logical(mask));

load('/Users/mumu/Documents/MATLAB/Polarizaiton_Imaging_Aid_Coarse_Depth/data_ball/3_leftball_PD.mat');
normal1 = double(normal1);
[Z_merged2,mask] = mergeDepthNormals(depthmap,normal1,P,k,logical(mask));

load('/Users/mumu/Documents/MATLAB/Polarizaiton_Imaging_Aid_Coarse_Depth/data_ball/3_leftball_PDS.mat');
normal1 = double(normal1);
...
% 切除点云多余的部分
maxZ = 652;
indices = points3D_depthmap(:, 3) <= maxZ;
points3D_depth = points3D_depthmap(indices, :);
indices = points3D_Z1_merged(:, 3) <= maxZ;
points3D_Z1 = points3D_Z1_merged(indices, :);
indices = points3D_Z2_merged(:, 3) <= maxZ;
points3D_Z2 = points3D_Z2_merged(indices, :);
indices = points3D_Z3_merged(:, 3) <= maxZ;
...
function [selected_points,z_negative] = plot_slice_convex_hull_with_center(points, center, r)
    ...
    % 筛选出y坐标接近y_center的点
    selected_points = points(abs(points(:,2) +50) < 0.1, :);  % 根据需要调整这个阈值
    x = selected_points(:,1);
