...
if __name__ == '__main__':
    ...
    data = sio.loadmat('/home/charlie/Desktop/Parallels Shared Folders/Polarizaiton_Imaging_Aid_Coarse_Depth/5_7/2_penguin9_O.mat')
    depthmap = data['depthmap']
    cam1 = data['cam1'][0][0]
    mask1 = cam1['mask']
    phi = cam1['phi_est']
    ...
    N_spec1 = CoordinateUtil.polar2normal(theta_spec[..., 0], phi + np.pi / 2)
    N_spec2 = CoordinateUtil.polar2normal(theta_spec[..., 0], phi - np.pi / 2)
    ...
    matplot.title('diffuse theta2')

    matplot.subplot(2, 2, 3)
    matplot.imshow((N_spec1+1)/2)
    matplot.title('speuclar theta 1')

    matplot.subplot(2, 2, 4)
    matplot.imshow((N_spec2 + 1) / 2)
    ...
    inf = opengm.inference.BeliefPropagation(gm, accumulator='minimizer',
                                             parameter=opengm.InfParam(steps=30, convergenceBound=0.0001, damping=0.5))
    #
    ...
    sio.savemat('/home/charlie/Desktop/Parallels Shared Folders/Polarizaiton_Imaging_Aid_Coarse_Depth/5_7/3_penguin9_ODS.mat', {'normal1': N_reco_full,
                                                             'depthmap': depthmap, 'mask': mask1, 'N_guide': N_guide})
